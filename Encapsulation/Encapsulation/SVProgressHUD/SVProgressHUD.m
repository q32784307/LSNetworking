...
- (void)setStatus:(NSString *)string {
    ...
    self.hudView.frame = CGRectMake(0, 0, 64, 64);
    UIImageView *gifImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 64, 64)];
    NSArray *gifArray = [NSArray arrayWithObjects:[UIImage imageNamed:@"myLoadingImage0"],
                         [UIImage imageNamed:@"myLoadingImage1"],
                         [UIImage imageNamed:@"myLoadingImage2"],
                         [UIImage imageNamed:@"myLoadingImage3"],nil];
    gifImageView.animationImages = gifArray; //动画图片数组
    gifImageView.animationDuration = 2; //执行一次完整动画所需的时长
    gifImageView.animationRepeatCount = 10;  //动画重复次数
    [gifImageView startAnimating];
    [self.hudView addSubview:gifImageView];
